...
-- Open full diagnostics in floating box
...
vim.keymap.set('n', '<left>', '<cmd>echo "Game is game"<CR>')
vim.keymap.set('n', '<right>', '<cmd>echo "Game is game"<CR>')
vim.keymap.set('n', '<up>', '<cmd>echo "Game is game"<CR>')
vim.keymap.set('n', '<down>', '<cmd>echo "Game is game"<CR>')
...
function _G.increment_first_number()
    -- Get the current line
    local line = vim.api.nvim_get_current_line()
    
    -- Find the first number in the line
    local start_pos, end_pos, number = string.find(line, "(%d+)")
    
    if number then
        -- Convert to number, increment, and convert back to string
        local new_number = tostring(tonumber(number) + 1)
        
        -- Replace the old number with the new one
        local new_line = string.sub(line, 1, start_pos - 1) .. 
                        new_number .. 
                        string.sub(line, end_pos + 1)
...
vim.keymap.set('n', '<leader>i', _G.increment_first_number, { noremap = true, silent = true })
...
require('lazy').setup({
  ...
  { 'Bilal2453/luvit-meta', lazy = true },
  {
    ...
    config = function()
--      -- Set up the LSP server for asm-lsp
--      local lspconfig = require('lspconfig')
--
--      lspconfig.asm_lsp.setup({
--        settings = {
--          asm = {
--            assembler = 'nasm',  -- Specify NASM as the assembler
--          },
--        },
--        -- Optional: Add any other configuration options for asm-lsp here
--      })
      ...
      local hls_config = {
        cmd = { "/run/current-system/sw/bin/haskell-language-server-wrapper", "-d", "--lsp" },
